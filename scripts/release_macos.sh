...
plugin_staging="$staging_directory/apple-debug-plugin"
./scripts/stage_codex_plugin.sh "$plugin_staging" "$root/.build/release/apple-debug-mcp"
plugin_binary="$plugin_staging/plugins/apple-debug/bin/apple-debug-mcp-bin"
codesign --force --options runtime --timestamp --sign "$identity" "$plugin_binary"
...
ditto "$plugin_staging" "$notary_payload/apple-debug-plugin"
...
mkdir -p "$distribution_directory/plugins" "$distribution_directory/.agents/plugins"
ditto "$plugin_staging/plugins/apple-debug" "$distribution_directory/plugins/apple-debug"
cp "$plugin_staging/.agents/plugins/marketplace.json" "$distribution_directory/.agents/plugins/marketplace.json"
