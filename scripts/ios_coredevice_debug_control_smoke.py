#!/usr/bin/env python3
"""Exercise an authorized modern iOS device through the MCP CoreDevice LLDB-DAP surface."""
...
def main() -> int:
    ...
    if os.environ.get("APPLE_DEBUG_ALLOW_DEVICE_DEBUG") != "1":
        print("ios-coredevice-debug-control-smoke: set APPLE_DEBUG_ALLOW_DEVICE_DEBUG=1", file=sys.stderr)
        return 1
    if os.environ.get("APPLE_DEBUG_ALLOW_DEVICE_MUTATION") != "1":
        print("ios-coredevice-debug-control-smoke: set APPLE_DEBUG_ALLOW_DEVICE_MUTATION=1", file=sys.stderr)
        return 1
    if os.environ.get("APPLE_DEBUG_ALLOW_EVALUATE") != "1":
        print("ios-coredevice-debug-control-smoke: set APPLE_DEBUG_ALLOW_EVALUATE=1", file=sys.stderr)
        return 1
    if os.environ.get("APPLE_DEBUG_ALLOW_MEMORY_WRITE") != "1":
        print("ios-coredevice-debug-control-smoke: set APPLE_DEBUG_ALLOW_MEMORY_WRITE=1", file=sys.stderr)
        return 1
    if not device_identifier or not Path(app_path).is_dir():
        print(
            "ios-coredevice-debug-control-smoke: set APPLE_DEBUG_COREDEVICE_ID and provide a signed .app",
            file=sys.stderr,
        )
        return 1
    ...
    if len(probe_lines) != 1:
        print("ios-coredevice-debug-control-smoke: deterministic control probe line is missing or ambiguous", file=sys.stderr)
        return 1
    ...
    def stopped_thread_and_frame() -> tuple[int, int, str]:
        ...
        if not threads:
            raise RuntimeError("CoreDevice LLDB-DAP returned no stopped threads")
        thread_id = threads[0]["id"]
        frames = require_body(
            tool("apple_debug_stack_trace", {"sessionID": session_id, "threadID": thread_id, "levels": 10})
        )["stackFrames"]
        if not frames:
            raise RuntimeError("CoreDevice LLDB-DAP returned no stopped stack frames")
    ...
    try:
        request(
            "initialize",
            {
                ...
                "capabilities": {},
                "clientInfo": {"name": "apple-debug-mcp-coredevice-control-smoke", "version": "0.1.0"},
            },
        )
        ...
        devices = tool("apple_device_list", {})
        device = next((item for item in devices if item.get("identifier") == device_identifier), None)
        if not device or not device.get("isAuthorizedForDevelopment"):
            raise RuntimeError(f"CoreDevice is not authorized after tunnel activation: {device}")

        tool("apple_device_install", {"identifier": device_identifier, "appPath": app_path})
        launch = tool(
            "apple_device_launch",
            {
        ...
        if not require_body(memory).get("data"):
            raise RuntimeError("CoreDevice LLDB-DAP returned no instruction bytes")
        tool(
        ...
        if not any(breakpoint.get("verified") for breakpoint in breakpoints):
            raise RuntimeError(f"CoreDevice source breakpoint was not verified: {breakpoint_response}")
        ...
        if not wait.get("stopped") or wait.get("terminated"):
            raise RuntimeError(f"CoreDevice breakpoint did not stop: {wait}")
        ...
        if "2" not in str(evaluation.get("result", "")):
            raise RuntimeError(f"CoreDevice expression evaluation returned an unexpected result: {evaluation}")
        ...
        if not step_wait.get("stopped") or step_wait.get("terminated"):
            raise RuntimeError(f"CoreDevice instruction step did not stop: {step_wait}")
        ...
        if not pause_wait.get("stopped") or pause_wait.get("terminated"):
            raise RuntimeError(f"CoreDevice pause did not produce a stop: {pause_wait}")
        ...
        if match is None:
            raise RuntimeError(f"CoreDevice debugger did not return a stack pointer: {stack_pointer}")
        ...
        if not patch.get("verified") or patch.get("rolledBack"):
            raise RuntimeError(f"CoreDevice memory patch was not verified: {patch}")
        restored = tool(
            "apple_debug_patch_memory",
            {
                "sessionID": session_id,
        ...
        if not restored.get("verified"):
            raise RuntimeError(f"CoreDevice memory rollback was not verified: {restored}")

        closed = tool("apple_debug_session_close", {"sessionID": session_id})
        session_id = None
        if not closed.get("closed"):
            raise RuntimeError("CoreDevice debug session did not close")
        print(
            "ios-coredevice-debug-control-smoke: install, launch PID, CoreDevice LLDB-DAP attach, threads, stack, registers, memory, disassembly, breakpoint, evaluate, step, pause/continue, memory patch/rollback, and cleanup passed"
        )
        return 0
    except Exception as error:
        print(f"ios-coredevice-debug-control-smoke: {error}", file=sys.stderr)
        return 1
    finally:
