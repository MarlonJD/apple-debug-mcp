...
| plugins/apple-debug/ | Repo-marketplace Codex plugin manifest, MCP wiring, workflow skill, and local executable launcher | Maintainers; update with plugin packaging or Codex workflow changes |
...
- The `Apple Debug` Codex plugin is a packaging and routing layer over the same MCP executable. Its `.mcp.json` starts `bin/apple-debug-mcp-launcher` over stdio; the launcher prefers a staged release executable, then an explicit executable override, local SwiftPM builds, or installed signed application bundles. The plugin does not alter MCP policy grants or replace the standalone MCP product.
