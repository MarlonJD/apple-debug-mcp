...
- [ ] (2026-08-26 16:54Z) Align Simulator video recording with the current public `simctl` default codec (`hevc`) and run the selected `simctl` directly with bounded signal shutdown/diagnostics after hosted macOS-26 produced an empty recording; accept a valid finalized movie after the intentional SIGINT even when hosted `simctl` reports a non-zero status, local Simulator MCP smoke is verified, and the complete Simulator tier remains pending the next remote run.
