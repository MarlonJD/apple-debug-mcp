...
- [x] (2026-08-26 21:19Z) Split the hosted Simulator integration workflow into `simulator-check-core` and a dedicated `simulator-repro-bundle` job with a separate 20-minute budget; the combined local `make simulator-check` passes both targets, while hosted completion awaits the next scheduled/manual run.
