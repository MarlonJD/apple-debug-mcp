...
- [x] (2026-08-27 21:52Z) Publish the hardening and plugin artifact as GitHub Release `v0.2.0` from commit `9402031`; the release is non-draft/non-prerelease and carries `apple-debug-mcp-macos-arm64-v0.2.0-notarized.zip` with SHA-256 `43e84dc0edf3054fb13acbe220a355b068960b1bcb4293908df6df772d6ebb85` after successful Developer ID/notarization/Gatekeeper validation and post-publication readback.
...
- [x] (2026-08-27 11:39Z) Re-run the modern CoreDevice physical-device evidence against the paired iPhone 17 (CoreDevice UUID `02329A9F-84C9-5499-9EBF-074EFCB45F7C`, physical UDID `00008150-001471191A0A401C`, iOS `27.0 (24A5424a)`): inventory reported `available (paired)`, `xcdevice` reported USB availability, `devicectl device info lockState` acquired the tunnel and reported `passcodeRequired=false`/`unlockedSinceBoot=true`, the signed fixture build passed, and the direct lifecycle plus LLDB-DAP control smokes passed. A later combined `make physical-device-check` retry at 11:41Z observed `passcodeRequired=true` and failed closed before launch with CoreDevice `Locked`; no unauthorized unlock or mutation bypass was attempted.
- [x] (2026-08-27 11:52Z) Re-run the complete modern physical-device wrapper while the paired iPhone 17 was unlocked: `make physical-device-check` passed its Swift build, CoreDevice lifecycle, LLDB-DAP install/launch/attach/control, memory patch/rollback, and cleanup stages; the wrapper reported `authorized legacy/CoreDevice integration passed`.
- [x] (2026-08-27 12:40Z) Add server-wide MCP `instructions` describing when to use Apple Debug MCP versus Build for iOS/macOS workflows, add safe Codex/Claude Code `mcp add` registration through `install_mcp.sh`, verify both client command shapes without touching real configuration, and include the helper in future unsigned/notarized archives.
...
- [x] (2026-08-27 18:20Z) Add a Claude Code plugin manifest and shared plugin MCP configuration, update the README/release guidance for Claude installation, and verify the combined agent plugin package.
...
- The same plugin root can serve Codex and Claude Code: each host gets its own manifest, while the root `skills/` and `.mcp.json` remain shared. Claude Code resolves the launcher through `CLAUDE_PLUGIN_ROOT`; Codex uses the plugin-root working directory.
...
- Decision: Adopt an AWS Agent Toolkit-style repo marketplace plugin named `apple-debug` while keeping `apple-debug-mcp` as the repository and canonical MCP server name.
  ...
  Date/Author: 2026-08-27 / Apple Debug MCP maintainers
- Decision: Add a `.claude-plugin/plugin.json` manifest to the same `apple-debug` package and share its skill/MCP configuration with Codex.
  Rationale: Claude Code plugins use the same root-level skills and `.mcp.json` pattern, so a second host manifest extends distribution without duplicating the Apple Debug implementation.
  Date/Author: 2026-08-27 / Apple Debug MCP maintainers
...
The repository also contains a repo-marketplace Codex/Claude Code plugin under `plugins/apple-debug/`. It packages shared workflow instructions and MCP wiring but does not replace the `AppleDebugMCP` executable or its authorization policy.
...
- Codex/Claude Code plugin evidence covers valid agent manifests, a repo marketplace, skill frontmatter, `.mcp.json`, source launcher, staged executable, and real stdio MCP initialize response;
...
- 2026-08-27: Published `v0.2.0` with adaptive verification, exact crash symbolication identity checks, the bundled Codex/Claude Code plugin, and the `install_mcp.sh` helper; the published arm64 asset passed digest, signature, staple, and Gatekeeper readback.
- 2026-08-27: Added the AWS Agent Toolkit-style repo marketplace and `apple-debug` Codex/Claude Code plugin with bundled MCP wiring, skill routing, executable fallback launcher, package/release staging, and plugin smoke verification.
