...
| Performance trace is unavailable or unparseable | apple_performance_record or apple_performance_analyze returns a typed xctrace, permission, export, or output error | Keep template, target, duration, schema, row count, and `.trace` path bounded; do not infer hotspots from a failed export | performance-analysis-smoke and tool schema smoke |
