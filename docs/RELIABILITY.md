...
| Semantic UI snapshot is empty | XCTest/XcodeBuildMCP returns no accessibility translation or stable targets | Keep explicit accessibility identifiers on fixture controls and re-run the Simulator build/snapshot flow; do not claim standalone MCP UI-tree support | XcodeBuildMCP snapshot observation |
