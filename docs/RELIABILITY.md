...
| Performance trace is unavailable | apple_performance_record returns a typed xctrace, permission, or output error | Keep template, target, duration, and `.trace` path bounded; do not infer profiling data from a failed command | DebugSessionTests and tool schema smoke |
