...
Current attestation source commit: 4f6bd20e4c1b7ed09f5b4554636ca691b00201cf.
...
| Logs, metrics, and traces are queryable when relevant | environment-contract.md and registry.md | CLI stderr or justified N/A | [N/A — metrics and traces do not apply to the short-lived foundation CLI](evidence/coverage-05.json) |
...
| Autonomy increases only after test, review, recovery, and escalation loops exist | operating-loop.md and output-contract.md | Evidence for granted level | [verified — foundation remains read-only with explicit escalation](evidence/coverage-16.json) |
...
| Per-worktree application isolation | Adopt separate worktrees for parallel agents; current foundation has no app state | Isolation proof | [verified — single-checkout isolation and separate-worktree policy are recorded](evidence/coverage-23.json) |
| Per-worktree observability stack | Reject; stdio transcript and stderr are sufficient for the short-lived CLI | Environment contract | [N/A — no long-lived service exists](evidence/coverage-24.json) |
...
| OpenAI's fixed layered domain architecture | Limit; use core, MCP, and future backend boundaries | Architecture and dependency test | [verified — project-specific core and backend boundaries are documented](evidence/coverage-27.json) |
...
| Automated merge and agent-authored release tooling | Reject | Release policy | [N/A — no release action exists](evidence/coverage-31.json) |
