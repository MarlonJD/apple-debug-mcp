...
| macOS desktop backend | python3 scripts/debug_fixture_smoke.py; make performance-analysis-smoke | make check | LLDB launch, source/instruction/function/exception breakpoints, breakpoint locations, stop snapshot, stop-event synchronization, modules, registers, stack/scopes, variables, completions, evaluate, memory/vmmap, xctrace rows/hotspots/flame stacks, disassembly, instruction stepping, continue, and cleanup | Keep target mutation opt-in and respect adapter capability support | Maintainers; backend addition |
| iOS Simulator backend | make ios-fixture-smoke; make xcode-artifact-smoke; make xcode-test-smoke | make ios-debug-fixture-smoke, make ios-ui-tree-smoke, and make ios-arbitrary-ui-smoke | Generic Xcode build returns app/dSYM artifacts; generic test returns xcresult summary; install/launch/screenshot, LLDB attach/inspection, project-backed and generated arbitrary-app accessibility trees, and UI-action evidence | Requires Xcode and, for target mutation, an available Simulator plus explicit mutation gate | Maintainers; backend addition |
