...
| Release | Unsigned package in CI; signed/notarized archive on an authorized Mac | `make package` / `make release-package` | Developer ID identity, notary profile, and Gatekeeper authority are external to the repository; GitHub Release `v0.2.0` carries the notarized, stapled, Gatekeeper-accepted arm64 archive and its published digest matched the local artifact | verified locally; published |
