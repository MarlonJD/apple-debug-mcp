...
| Plugin host | Candidate executables must pass codesign audit and optional team-ID matching; execution is isolated in the separate deny-by-default/no-network host and requires an explicit grant | `ApplePluginHostService`, `apple-debug-plugin-host` | plugin-smoke |
